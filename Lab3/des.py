...
class des():
    ...
    def substitute(self, d_e):#Substitute bytes using SBOX
        sblocks = nsplit(d_e, 6)  
        count = 0  
        result = []  

        for i in sblocks:  
            temp = self.compute_s_box(i, count) 
    ...
    def compute_s_box(self,block,round):
        blockstr = ''
        for i in block:
            blockstr = blockstr + str(i) 
        row = int(blockstr[0] + blockstr[5], 2)  
        column = int(blockstr[1:5], 2) 

        return binvalue(S_BOX[round][row][column], 4) 
